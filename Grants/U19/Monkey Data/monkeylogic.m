...
%         set(hTag.MinifyRuntime,'value',MLConfig.MinifyRuntime);
    ...
    function update_ioUI()
        ...
        set(hTag.RewardFuncArgs,'string',sprintf('%d ms, %d time(s)',MLConfig.RewardFuncArgs.Duration,MLConfig.RewardFuncArgs.NumReward));
        
        set(hTag.EditRewardArgs,'enable',fi(isempty(MLPath.RewardFunction),'off','on'));
        set(hTag.EditRewardFunc,'enable',fi(isempty(MLPath.RewardFunction),'off','on'));
    ...
    function UIcallback(hObject,~)
        ...
        switch obj_tag
            case {'SubjectScreenDevice','ForcedUseOfFallbackScreen','MouseCursorType','PhotoDiodeTrigger', ...
            ...
            case 'JoystickCursorImage'
                [filename,filepath] = uigetfile({'*.bmp;*.gif;*.jpg;*.jpeg;*.tif;*.tiff;*.png;*.avi;*.mpg;*.mpeg','Image/Movie Files'; '*.*','All Files'},'Select a(n) image/movie file',fileparts(MLConfig.(obj_tag){MLConfig.JoystickNumber}));
            ...
            case 'StrobeTest'
                ...
                destroy(DAQ);
                set(gcbo,'enable','on');
            case 'EditRewardArgs', DlgEditRewardArgs();
            case 'RewardTest'
                try
            ...
            case 'SubjectName'
                try
                    ...
                    elseif namelengthlimit < length(val)
                        ...
                        check_cfg_change();
                        if isempty(val), val = ''; else, loadcfg(MLPath.ConfigurationFile,['MLConfig_' lower(val)]); end
    ...
    function DlgEditRewardArgs()
        ...
        try
            ...
            tag(1) = uicontrol('parent',hDlg,'style','edit','position',[140 198 100 25],'tag','RewardJuiceLine','string',num2str(MLConfig.RewardFuncArgs.JuiceLine),'fontsize',fontsize);
            tag(2) = uicontrol('parent',hDlg,'style','edit','position',[140 168 100 25],'tag','RewardDuration','string',num2str(MLConfig.RewardFuncArgs.Duration),'fontsize',fontsize);
            tag(3) = uicontrol('parent',hDlg,'style','edit','position',[140 138 100 25],'tag','RewardNumReward','string',num2str(MLConfig.RewardFuncArgs.NumReward),'fontsize',fontsize);
            tag(4) = uicontrol('parent',hDlg,'style','edit','position',[140 108 100 25],'tag','RewardPauseTime','string',num2str(MLConfig.RewardFuncArgs.PauseTime),'fontsize',fontsize);
            tag(5) = uicontrol('parent',hDlg,'style','edit','position',[140 78 100 25],'tag','RewardTriggerVal','string',num2str(MLConfig.RewardFuncArgs.TriggerVal),'fontsize',fontsize);
            tag(6) = uicontrol('parent',hDlg,'style','edit','position',[120 48 120 25],'tag','RewardCustom','string',MLConfig.RewardFuncArgs.Custom,'fontsize',fontsize);

            exit_code = 0; dlg_wait();
            if 1==exit_code
                MLConfig.RewardFuncArgs.JuiceLine = str2double(get(tag(1),'string'));
                MLConfig.RewardFuncArgs.Duration = str2double(get(tag(2),'string'));
                MLConfig.RewardFuncArgs.NumReward = str2double(get(tag(3),'string'));
                MLConfig.RewardFuncArgs.PauseTime = str2double(get(tag(4),'string'));
                MLConfig.RewardFuncArgs.TriggerVal = str2double(get(tag(5),'string'));
                MLConfig.RewardFuncArgs.Custom = get(tag(6),'string');
            end
        catch err
            warning_handler(err);
        end
    ...
    function DlgOtherDeviceSettings()
        ...
        audio_engine = {'XAudio2','WASAPI Shared','WASAPI Exclusive'};
        audio_engine_idx = [1 2 3];
		if isempty(info) || 0==info.NumExclusiveDevices, audio_engine(3) = []; audio_engine_idx(3) = []; end
		if isempty(info) || 0==info.NumSharedDevices, audio_engine(2) = []; audio_engine_idx(2) = []; end
        ...
        eyeid = {'None',''; 'Arrington ViewPoint EyeTracker','viewpoint'; 'SR Research EyeLink','eyelink'; 'ISCAN Eye Tracker','iscan'}; eyeid = eyeid([true; ismember(eyeid(2:end,2),mdqmex(20,1,2))],:);
        joyid = {'None',''}; if any(strcmpi('joystick',daqhwinfo('all'))), info = daqhwinfo('joystick'); for m=length(info.InstalledBoardIds):-1:1, joyid(m+1,:) = {sprintf('%s: %s',info.InstalledBoardIds{m},info.BoardNames{m}),info.InstalledBoardIds{m}}; end, end
        camid = {'None',''}; if any(strcmpi('webcam',daqhwinfo('all'))), info = daqhwinfo('webcam'); for m=length(info.InstalledBoardIds):-1:1, camid(m+1,:) = {sprintf('%s: %s',info.InstalledBoardIds{m},info.BoardNames{m}),info.InstalledBoardIds{m}}; end, end
        comid = {'None',''}; if any(strcmpi('serial',daqhwinfo('all'))), info = daqhwinfo('serial'); for m=length(info.InstalledBoardIds):-1:1, comid(m+1,:) = {info.BoardNames{m},info.InstalledBoardIds{m}}; end, end
        voiceid = {'None',''}; if any(strcmpi('wasapi',daqhwinfo('all'))), info = daqhwinfo('wasapi'); for m=length(info.InstalledBoardIds):-1:1, voiceid(m+1,:) = {sprintf('%s: %s',info.InstalledBoardIds{m},info.BoardNames{m}),info.InstalledBoardIds{m}}; end, end
        hDev = struct;
        ...
        try
            bgcolor = [0.9255 0.9137 0.8471];
            set(hDlg,'tag','DlgOtherDeviceSettings','units','pixels','position',pos,'menubar','none','numbertitle','off','name','Other device settings','color',bgcolor,'windowstyle','modal');
            callback = @dlg_proc;
            ...
            hDev.AudioEngine(1) = uicontrol('parent',hDlg,'style','popupmenu','position',[x0+125 y0+7 135 22],'string',audio_engine,'value',fi(isempty(info),1,AudioEngine.ID),'fontsize',fontsize,'callback',callback);
            hDev.AudioEngine(6) = uicontrol('parent',hDlg,'style','pushbutton','position',[x0+265 y0+5 25 25],'string','...','fontsize',fontsize,'callback','system(''mmsys.cpl'');');
            hDev.AudioEngine(7) = uicontrol('parent',hDlg,'style','pushbutton','position',[x0+295 y0+5 25 25],'tag','PlaySound','enable',fi(exist(test_sound,'file'),'on','off'),'cdata',play_button,'callback',callback,'tooltip','Play a test sound');
            hDev.AudioEngine(8) = uicontrol('parent',hDlg,'style','pushbutton','position',[x0+335 y0+9 17 17],'tag','RefreshDev','cdata',refresh_icon,'callback',callback,'tooltip','Reload device lists (F5)');

            y0 = y0-27;
            uicontrol('parent',hDlg,'style','text','position',[x0+75 y0+3 45 22],'string','Device','backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold');
            hDev.AudioEngine(2) = uicontrol('parent',hDlg,'style','edit','position',[x0+125 y0+7 230 22],'string',deviceName,'enable','inactive','fontsize',fontsize);
            hDev.AudioEngine(3) = uicontrol('parent',hDlg,'style','popupmenu','position',[x0+125 y0+7 230 22],'string',deviceName,'value',fi(isempty(info),1,AudioEngine.Device),'fontsize',fontsize,'callback',callback);

            y0 = y0-27;
            uicontrol('parent',hDlg,'style','text','position',[x0+75 y0+3 45 22],'string','Format','backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold');
            hDev.AudioEngine(4) = uicontrol('parent',hDlg,'style','edit','position',[x0+125 y0+7 230 22],'string',formatString,'enable','inactive','fontsize',fontsize);
            hDev.AudioEngine(5) = uicontrol('parent',hDlg,'style','popupmenu','position',[x0+125 y0+7 230 22],'string',formatString,'value',AudioEngine.Format,'fontsize',fontsize,'callback',callback);
            ...
            uicontrol('parent',hDlg,'style','pushbutton','position',[x0+275 y0+5 25 25],'string','...','fontsize',fontsize,'callback','system(''control'');');

            y0 = y0-30;
            uicontrol('parent',hDlg,'style','text','position',[x0 y0 200 25],'string','USB Joystick','backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold','horizontalalignment','left');
            uicontrol('parent',hDlg,'style','text','position',[x0+315 y0+20 40 25],'string','# Btn','backgroundcolor',bgcolor,'fontsize',fontsize);
            for m=1:length(MLConfig.USBJoystick)
                ...
                hDev.USBJoystick(m,1) = uicontrol('parent',hDlg,'style','popupmenu','position',[x0+125 y0+7 185 22],'tag',sprintf('JoystickList%d',m),'string',joyid(:,1),'fontsize',fontsize,'callback',callback);
                hDev.USBJoystick(m,2) = uicontrol('parent',hDlg,'style','popupmenu','position', [x0+315 y0+7 40 22],'string',0,'fontsize',fontsize);
                y0 = y0-30;
            end
            ...
            hDev.WebcamExportAs = uicontrol('parent',hDlg,'style','popupmenu','position',[x0+140 y1 185 22],'tag','WebcamExportAs','string',{'to data file (no compression)','to data file (compression)','as separate MP4 files'},'value',MLConfig.WebcamExportAs,'fontsize',fontsize,'callback',callback);
            y0 = y0-30; y1 = y1-30;
            ...
            y0 = y0-30;
            uicontrol('parent',hDlg,'style','text','position',[x0 y0 200 25],'string','Reward via Bluetooth','backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold','horizontalalignment','left');
            ...
            uicontrol('parent',hDlg,'style','text','position',[0 50 pos(3) 25],'string','* These devices have priority over DAQ devices.','backgroundcolor',bgcolor,'fontsize',fontsize+1);
            ...
            for m=1:length(MLConfig.USBJoystick)
                ...
                if isempty(MLConfig.USBJoystick(m).ID), continue, end
                try
                    hwInfo = daqhwinfo(pointingdevice('joystick',MLConfig.USBJoystick(m).ID));
                    set(hDev.USBJoystick(m,2),'string',0:hwInfo.Buttons,'value',min(hwInfo.Buttons,MLConfig.USBJoystick(m).NumButton)+1);
                catch
                    % do nothing
                end
            end
            ...
            if 1==exit_code
                ...
                for m=1:length(MLConfig.USBJoystick)
                    ...
                    MLConfig.USBJoystick(m).NumButton = get(hDev.USBJoystick(m,2),'value')-1;
                end
        ...
        function refresh_devices()
            err = [];
            try
                set(hDev.AudioEngine(8),'enable','inactive','cdata',refresh_active); drawnow;
                mglreset; if ~isempty(hTag.hVideo), set(hTag.SubjectScreenDevice,'string',num2cell(1:System.NumberOfScreenDevices),'value',min(get(hTag.SubjectScreenDevice,'value'),System.NumberOfScreenDevices)); end
                daqreset; refresh_boards();
            catch err
            end
        ...
        function dlg_proc(hObject,~)
            ...
            switch obj_tag(1:min(length(obj_tag),10))
                ...
                case 'JoystickLi'
                    ...
                    id = get_selected_id(hDev.USBJoystick(no,1),joyid);
                    if ~isempty(id)
                        try
                            hwInfo = daqhwinfo(pointingdevice('joystick',id));
                            set(hDev.USBJoystick(no,2),'string',0:hwInfo.Buttons,'value',min(hwInfo.Buttons,MLConfig.USBJoystick(no).NumButton)+1);
                        catch
                            % do nothing
                        end
                    end
                ...
                case 'EyeTracker'
                    hDev.EyeTrackerProperty = PopupEyeTrackerSetup(hDev.EyeTrackerProperty,eyeid(get(hDev.EyeTracker(1),'value'),:));
                case 'SerialPort'
                    hDev.SerialPortProperty = PopupSerialSetup(hDev.SerialPortProperty);
                case 'VoiceRecor'
                    hDev.VoiceRecordingProperty = PopupVoiceRecordingSetup(hDev.VoiceRecordingProperty);
            end
            dlg_update();
        end
        function dlg_update()
            try
                ...
                set(hDev.AudioEngine(1:7),'enable','off');
            end
            ...
            set(hDev.SerialPort(2),'enable',fi(1==get(hDev.SerialPort(1),'value'),'off','on'));
            set(hDev.VoiceRecording(2),'enable',fi(1==get(hDev.VoiceRecording(1),'value'),'off','on'));
        end
    end

    function prop = PopupEyeTrackerSetup(prop,id)
        old_prop = prop;
        tracker = id{1};
        eyeid = id{2};
        ...
        try
            ...
            switch eyeid
                ...
                case 'iscan'
                    hc.IP_address(1) = uicontrol('parent',hPop,'style','text','position',[x0-5 y0-30 200 25],'string','IP address','backgroundcolor',bgcolor,'fontsize',fontsize,'horizontalalignment','left');
                    hc.IP_address(2) = uicontrol('parent',hPop,'style','edit','position',[x0+65 y0-25 130 25],'string',prop.ISCAN.IP_address,'fontsize',fontsize);
                    hc.Port(1) = uicontrol('parent',hPop,'style','text','position',[x0-5 y0-60 200 25],'string','Port','backgroundcolor',bgcolor,'fontsize',fontsize,'horizontalalignment','left');
                    hc.Port(2) = uicontrol('parent',hPop,'style','edit','position',[x0+65 y0-55 130 25],'string',prop.ISCAN.Port,'fontsize',fontsize);
                    hc.Binocular(1) = uicontrol('parent',hPop,'style','text','position',[x0-5 y0-90 200 25],'string','Binocular','backgroundcolor',bgcolor,'fontsize',fontsize,'horizontalalignment','left');
                    hc.Binocular(2) = uicontrol('parent',hPop,'style','checkbox','position',[x0+70 y0-80 15 15],'tag','Binocular','value',prop.ISCAN.Binocular,'callback',callback);
                    x1 = x0; y1 = y0-120;
                    ...
                    hc.Test(1) = uicontrol('parent',hPop,'style','pushbutton','position',[x0+205 y0-26 120 25],'string','Connection Test','fontsize',fontsize,'callback',@test_eyetracker_connection);
                    hc.Test(2) = uicontrol('parent',hPop,'style','text','position',[x0+205 y0-60 120 25],'string','','backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold','horizontalalignment','center');
                    uicontrol('parent',hPop,'style','pushbutton','position',[w-240 10 70 25],'string','Help','fontsize',fontsize,'callback',['web(''' MLPath.DocDirectory 'docs_ISCAN_UDP_setup.html'',''-browser'')']);
            ...
            try
                switch eyeid
                    ...
                    case 'iscan'
                        ...
                        set(hc.Test(2),'string',fi(connected,'Success!!!','Failed!!!'),'foregroundcolor',fi(connected,[0 1 0],[1 0 0]));
                end
            catch
                % do nothing
            end
    ...
    function prop = PopupSerialSetup(prop)
        old_prop = prop;
        ...
        try
            x0 = 10; y0 = h-40;
            uicontrol('parent',hPop,'style','text','position',[x0 y0 80 25],'string','Baud rate','backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold','horizontalalignment','left');
            hc(1) = uicontrol('parent',hPop,'style','popupmenu','position',[x0+80 y0+5 80 22],'string',baudrate,'value',find(cellfun(@(x)x==prop.BaudRate, baudrate),1),'fontsize',fontsize);
            y0 = y0 - 30;
            uicontrol('parent',hPop,'style','text','position',[x0 y0 80 25],'string','Byte size','backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold','horizontalalignment','left');
            hc(2) = uicontrol('parent',hPop,'style','popupmenu','position',[x0+80 y0+5 40 22],'string',bytesize,'value',find(cellfun(@(x)x==prop.ByteSize, bytesize),1),'fontsize',fontsize);
            y0 = y0 - 30;
            uicontrol('parent',hPop,'style','text','position',[x0 y0 80 25],'string','Stop bits','backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold','horizontalalignment','left');
            hc(3) = uicontrol('parent',hPop,'style','popupmenu','position',[x0+80 y0+5 100 22],'string',stopbits,'value',find(strcmpi(stopbits,prop.StopBits),1),'fontsize',fontsize);
            y0 = y0 - 30;
            uicontrol('parent',hPop,'style','text','position',[x0 y0 80 25],'string','Parity','backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold','horizontalalignment','left');
            hc(4) = uicontrol('parent',hPop,'style','popupmenu','position',[x0+80 y0+5 100 22],'string',parity,'value',find(strcmpi(parity,prop.Parity),1),'fontsize',fontsize);
            uicontrol('parent',hPop,'style','pushbutton','position',[w-160 10 70 25],'tag','done','string','Done','fontsize',fontsize,'callback',callback);
            uicontrol('parent',hPop,'style','pushbutton','position',[w-80 10 70 25],'tag','cancel','string','Cancel','fontsize',fontsize,'callback',callback);

            pop_exit = 0; pop_wait();
            if 1==pop_exit
                prop.BaudRate = baudrate{get(hc(1),'value')};
                prop.ByteSize = bytesize{get(hc(2),'value')};
                prop.StopBits = stopbits{get(hc(3),'value')};
                prop.Parity   = parity{get(hc(4),'value')};
            else
                prop = old_prop;
            end
        catch err
        ...
        try
            switch lower(stim{1})
                ...
                case 'gen'
                    ...
                    trialrecord = mltrialrecord(MLConfig).new_trial();
                    try
    ...
    function init()
        ...
        if System.OSVersion(1) < 10, disp([newline 'This version of NIMH MonkeyLogic requires <strong>Windows 10</strong>.' newline 'For earlier Windows, install the previous version (v2.0).' newline '<a href="https://monkeylogic.nimh.nih.gov/download.html">https://monkeylogic.nimh.nih.gov/download.html</a>']); return, end
        ...
        addpath(MLPath.BaseDirectory,[MLPath.BaseDirectory 'daqtoolbox'],[MLPath.BaseDirectory 'mgl'],[MLPath.BaseDirectory 'kbd'],[MLPath.BaseDirectory 'ext'], ...
        ...
        try
            ...
            if ~isempty(strfind(err.identifier,'nvalidMEXFile'))
                msg = [newline 'NIMH MonkeyLogic requires <strong>Microsoft Visual C++ Redistributable' newline 'for Visual Studio 2015, 2017 and 2019</strong>.' newline ...
                    'Here is a direct link to the package. Please install it first.' newline ...
                    '<a href="https://aka.ms/vs/16/release/vc_redist.x64.exe">https://aka.ms/vs/16/release/vc_redist.x64.exe</a>' newline newline ...
                    ...
                    '<a href="https://support.microsoft.com/en-us/help/2977003/the-latest-supported-visual-c-downloads">https://support.microsoft.com/en-us/help/2977003/the-latest-supported-visual-c-downloads</a>' newline ...
                    ];
                fprintf(2,'%s\n',msg);
                return
            end
    ...
    function init_menu()
        ...
        if screen_pos(3) < 900 || screen_pos(4) < 880, collapsed_menu = true; end
        ...
        hTag.hFig = figure;
        set(hTag.hFig,'tag','mlmainmenu','units','pixels','position',fig_pos,'numbertitle','off','name',sprintf('NIMH MonkeyLogic %s',MLConfig.MLVersion),'menubar','none','resize','off','color',figure_bgcolor);
...
%         hTag.MinifyRuntime = uicontrol('style','checkbox','position',[x+95 y+6 15 15],'tag','MinifyRuntime','backgroundcolor',bgcolor,'callback',callbackfunc);
    ...
    function menu_video(x0,y0)
        ...
        hTag.IORefresh1 = uicontrol('style','pushbutton','position',[x+155 y+12 17 17],'tag','IORefresh','cdata',refresh_icon,'callback',callbackfunc,'tooltip','Refresh video boards');
        ...
        hTag.DiagonalSize = uicontrol('style','edit','position',[x+140 y+3 55 22],'tag','DiagonalSize','fontsize',fontsize,'callback',callbackfunc,'tooltip','Diagonal size of the subject screen');
        y = y - 25;
        ...
        hTag.ViewingDistance = uicontrol('style','edit','position',[x+140 y+3 55 22],'tag','ViewingDistance','fontsize',fontsize,'callback',callbackfunc,'tooltip','Distance between the subject''eye and the screen');
        y = y - 25;
        ...
        hTag.PixelsPerDegree = uicontrol('style','edit','position',[x+140 y+3 55 22],'tag','PixelsPerDegree','enable','inactive','backgroundcolor',bgcolor,'fontsize',fontsize);
        hTag.VideoTest = uicontrol('style','pushbutton','position',[x+203 y+2 58 124],'tag','VideoTest','string','Test','fontsize',fontsize,'callback',callbackfunc,'tooltip','Test the selected subject screen device');
        ...
        hTag.SubjectScreenBackground = uicontrol('style','pushbutton','position',[x+165 y+3 100 22],'tag','SubjectScreenBackground','string','Color','fontsize',fontsize,'callback',callbackfunc);
        y = y - 29;
    ...
    function menu_io(x0,y0)
        ...
        x = x0 + 5; y = y - 144;
        uicontrol('style','text','position',[x y+115 135 22],'string','Signal type','backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold');
        hTag.SignalType = uicontrol('style','listbox','position',[x y 135 120],'tag','SignalType','string',IOName,'fontsize',fontsize,'callback',callbackfunc);
        uicontrol('style','text','position',[x+140 y+115 150 22],'string','I/O boards','backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold');
        hTag.IORefresh2 = uicontrol('style','pushbutton','position',[x+270 y+125 17 17],'tag','IORefresh','cdata',refresh_icon,'callback',callbackfunc,'tooltip','Refresh I/O boards');
        hTag.IOBoards = uicontrol('style','listbox','position',[x+140 y 150 120],'tag','IOBoards','fontsize',fontsize,'callback',callbackfunc);
        y = y - 115;
        ...
        x = x0 + 10; y = y - 30;
        uicontrol('style','text','position',[x y 250 22],'string','Other device settings (USB, etc.)','backgroundcolor',bgcolor,'fontsize',fontsize,'fontweight','bold','horizontalalignment','left');
        ...
        hTag.RewardFuncArgs = uicontrol('style','edit','position',[x+50 y+3 110 22],'tag','RewardFuncArgs','enable','inactive','backgroundcolor',bgcolor,'fontsize',fontsize,'callback',callbackfunc,'tooltip',['Additional arguments for the custom reward function.' newline 'Leave it blank if you don''t have any custom variable.']);
        hTag.EditRewardArgs = uicontrol('style','pushbutton','position',[x+165 y+3 55 22],'tag','EditRewardArgs','string','Args','fontsize',fontsize,'callback',callbackfunc,'tooltip','Enter the reward variables');
        hTag.EditRewardFunc = uicontrol('style','pushbutton','position',[x+225 y+3 55 22],'tag','EditRewardFunc','string','Edit','fontsize',fontsize,'callback',callbackfunc,'tooltip','Edit the custom reward function');
        y = y - 25;
    ...
    function loadcfg(filepath,config_by_subject)
        ...
        if ~isa(content.(config_by_subject),'mlconfig') && ~isfield(content.(config_by_subject),'ImportFromStruct'), error('Invalid config file: %s',strip_path(filepath)); end
