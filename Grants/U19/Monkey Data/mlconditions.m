classdef mlconditions < handle
    ...
    methods
            ...
            for m=1:nstr
                switch lower(str{m}(1:3))
                    ...
                    case 'pic'  % pic(filename,Xpos,Ypos[,colorkey]) or pic(filename,Xpos,Ypos,width,height[,colorkey])
                        ...
                        if ~isempty(tokens)
                            [p,n] = find_ext(obj,tokens{1}{2},{'.bmp','.jpg','.jpeg','.tif','.tiff','.png','.gif'});
                            if isempty(p), error('PIC: Can''t find ''%s''',tokens{1}{2}); else, tokens{1}{2} = p; end
                            try mglimread(p); catch err, error('PIC: %s (%s)',err.message, n); end
                    ...
                    case 'mov'  % mov(filename,Xpos,Ypos[,looping])
                        ...
                        if ~isempty(tokens)
                            ...
                            try mgldestroygraphic(mgladdmovie(p)); catch err, error('MOV: %s (%s)',err.message,n); end
                            if isempty(tokens{1}{end}), tokens{1} = tokens{1}(1:end-1); end
                            tokens{1}(3:end) = str2num(obj,tokens{1}(3:end));
                            b = sprintf('MOV: %s',n);
                        end
                    ...
                    case 'snd'  % snd(filename) or snd(sin,duration,frequency)
                        ...
                        if ~isempty(tokens)
                            if 2==length(tokens{1})
                                [p,n] = find_ext(obj,tokens{1}{2},{'.wav','.mat','.mp3','.ogg','.avi'});
                                if isempty(p), error('SND: Can''t find ''%s''',tokens{1}{2}); else, tokens{1}{2} = p; end
                                try y = load_waveform(tokens{1}); if isscalar(y), mgldestroysound(y); end, catch err, error('SND: %s (%s)',err.message,n); end
                                b = sprintf('SND: %s',n);
                            else
                                tokens{1}(2:end) = str2num(obj,tokens{1}(2:end));
                    ...
                    case 'stm'  % stm(port,datasource[,retriggerable])
                        ...
                        if ~isempty(tokens)
                            [p,n] = find_ext(obj,tokens{1}{3},{'.wav','.mat','.mp3','.ogg','.avi'});
                            if isempty(p), error('STM: Can''t find ''%s''',tokens{1}{3}); else, tokens{1}{3} = p; end
                            try load_waveform({'stm',p}); catch err, error('STM: %s (%s)',err.message,n); end
                            if isempty(tokens{1}{end}), tokens{1} = tokens{1}(1:end-1); end
                    ...
                    case 'gen'  % gen(function_name) or gen(function_name,Xpos,Ypos)
                        ...
                        if ~isempty(tokens)
                            ...
                            try get_function_handle(p); catch err, error('GEN: %s (%s)',err.message,n); end
        ...
        function [p,n] = find_ext(obj,name,ext_list)
            ...
            for ext = ext_list(:)'
                [p,n] = mlsetpath([name ext{1}],{obj.MLPath.ExperimentDirectory,obj.MLPath.BaseDirectory});
