function data = parse_amazon_turk(main_dir,task)

switch task
    case 'sd'
        ver_folder = '/sd_version/';
        data_folder = '/sd_data/';

    case 'ccgp'
        ver_folder = '/ccgp_version/';
        data_folder = '/ccgp_data/';
        
    case 'efgh'
        ver_folder = '/efgh_version/';
        data_folder = '/efgh_data/';
...
versions        = load_available_versions([main_dir,ver_folder]);

% Load the subject data
folder           = [main_dir,data_folder];
data             = struct();
...
for j=1:length(filename)
    ...
    % load the task file
    task_data    = readcell([main_dir,ver_folder,task]);
    
    gt           = tbl.correct_response(idx_response);
    ...
    data(j).task_file      = task;
    data(j).task      = task_data;
    data(j).date      = file_date{j};
    data(j).sessionID =  subjects{j};
    data(j).stim      = stim;
    data(j).gt        = gt;
    data(j).key_press = remap(key_press);
    data(j).rt        = rt;
