...
con1.add(tc);
if TrialRecord.User.CC || z<0.001
...
con5 = Concurrent(lh5);
con5.add(tc);
scene5 = create_scene(con5,[fixation_point target off_target1 off_target2]);
...
con6a.add(tc);
scene6a = create_scene(con6a,[target off_target1 off_target2]);

% scene 6: choice
mul6 = MultiTarget(eye_);
mul6.Target = [target off_target1 off_target2];
...
con6.add(tc);
scene6 = create_scene(con6,[target off_target1 off_target2]);
...
%% REWARDS
trial_correct = target==mul6.ChosenTarget; % (ismember(TrialRecord.CurrentCondition, [1 5 9]) && mul6.ChosenTarget == up) || (ismember(TrialRecord.CurrentCondition, [2 6 7]) && mul6.ChosenTarget == right) || (ismember(TrialRecord.CurrentCondition, [3 4 8]) && mul6.ChosenTarget == left);
small_reward_trial = ismember(TrialRecord.CurrentCondition,[2 3 4 5 7 9]); % Test for large/ small trial; gd 3/30/22
...
if trial_correct
    ...
    if small_reward_trial % Test for large/ small trial; gd 3/30/22
        ...
        goodmonkey(solenoid_time*rMult, 'numreward', little_drops, 'pausetime', drop_gaps, 'eventmarker',99);
        TrialRecord.User.RewardGiven = [TrialRecord.User.RewardGiven; solenoid_time*rMult*little_drops];
    else
        idle(decision_trace_time,[], 70)
        sound(TrialRecord.User.rbfx, TrialRecord.User.rbFs)
        goodmonkey(solenoid_time*rMult, 'numreward', big_drops, 'pausetime', drop_gaps, 'eventmarker',99);
        TrialRecord.User.RewardGiven = [TrialRecord.User.RewardGiven; solenoid_time*rMult*big_drops];
    end
    
elseif (mul6.ChosenTarget == off_target1 && ismember(TrialRecord.CurrentCondition, [3 4 8])) || (mul6.ChosenTarget == off_target2 && ismember(TrialRecord.CurrentCondition, [2 6 7]))
    trialerror(1); % up error
...
elseif (mul6.ChosenTarget == off_target1 && ismember(TrialRecord.CurrentCondition, [1 5 9])) || (mul6.ChosenTarget == off_target2 && ismember(TrialRecord.CurrentCondition, [3 4 8]))
    trialerror(2); % right error
    TrialRecord.User.cond_count(TrialRecord.CurrentCondition) = TrialRecord.User.cond_count(TrialRecord.CurrentCondition) + 1; % 09/22/23 GD: increment cond_counter for CL
    TrialRecord.User.RewardGiven = [TrialRecord.User.RewardGiven; 0];
    sound(TrialRecord.User.infx, TrialRecord.User.inFs)
    if small_reward_trial
...
elseif (mul6.ChosenTarget == off_target1 && ismember(TrialRecord.CurrentCondition, [2 6 7])) || (mul6.ChosenTarget == off_target2 && ismember(TrialRecord.CurrentCondition, [1 5 9]))
    trialerror(4); % left error
    TrialRecord.User.cond_count(TrialRecord.CurrentCondition) = TrialRecord.User.cond_count(TrialRecord.CurrentCondition) + 1; % 09/22/23 GD: increment cond_counter for CL
    TrialRecord.User.RewardGiven = [TrialRecord.User.RewardGiven; 0];
    sound(TrialRecord.User.infx, TrialRecord.User.inFs)
    if small_reward_trial
